...
bool shift = 0;   // > 1 if the shift key is pressed
int ctrl = 0;

char unread_buffer[500];

void read_keyboard(char* buffer, size_t size){
    memcpy(buffer, unread_buffer, size);
    memset(unread_buffer, 0, 500);
}
...
void process_key_press(uint8_t scan_code) {
    ...
    if (strlen(unread_buffer) < 499){
        unread_buffer[strlen(unread_buffer)-1] = c;
    }
    else
    {
        memset(unread_buffer, 0, 500);
